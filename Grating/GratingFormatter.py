...
# TODOne: Fix cut_spacing output. 4/4/19 - Fixed. Added '+ 1' to 'steps' in path_x and path_z definitions
# TODOne: Fix output; it's appending. 4/5/19 - Fixed. Changed 'a+' to 'w+'
...
[profile_x, profile_z] = x_z_to_nc(ncmainfilename)
print(profile_x)
print(profile_z)
...
for index in profile_index:
    ...
    path_x = np.linspace(start_x, end_x, steps + 1)[:-1]  # do [:-1] to avoid duplicate coordinates at start/end points   # additional '-1' added to steps for testing 4-4-19
    path_z = np.linspace(start_z, end_z, steps + 1)[:-1]
    path_index = np.linspace(0, steps, steps + 1)[:-1]
    #    print(path_index)
